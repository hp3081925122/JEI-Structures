...
public final class EmiStructureRecipeOpener {
    ...
    public static boolean open(StructureRecipe targetRecipe) {
        ...
        try {
            ...
            var sourceId = targetRecipe.getId();
            var emiRecipeId = net.minecraft.resources.ResourceLocation.fromNamespaceAndPath(sourceId.getNamespace(), "/" + sourceId.getPath());
            Object recipe = emiRecipeManagerClass.getMethod("getRecipe", net.minecraft.resources.ResourceLocation.class).invoke(recipeManager, emiRecipeId);
            if (recipe == null) {
